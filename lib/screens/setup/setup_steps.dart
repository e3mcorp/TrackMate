    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          Text(
            localizations?.get('welcomeDescription') ?? 'TrackMate ti aiuta a monitorare i tuoi veicoli tramite tracker GPS.\n\nRiceverai SMS con posizioni, stato della batteria e altre informazioni utili.',
            textAlign: TextAlign.center,
              ...
              child: Column(
                ...
                children: [
                  _FeatureItem(
                    ...
                    text: localizations?.get('realTimePositions') ?? 'Posizioni in tempo reale',
                    colorScheme: colorScheme,
                  ),
                  _FeatureItem(
                    icon: Icons.battery_std,
                    text: localizations?.get('batteryMonitoring') ?? 'Monitoraggio batteria',
                    colorScheme: colorScheme,
                  ),
                  _FeatureItem(
                    icon: Icons.route,
                    text: localizations?.get('routeHistory') ?? 'Storico percorsi',
                    colorScheme: colorScheme,
                  ),
                  _FeatureItem(
                    icon: Icons.notifications,
                    text: localizations?.get('automaticNotifications') ?? 'Notifiche automatiche',
                    colorScheme: colorScheme,
                  ),
                ],
              ),
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            // ✅ Numero SIM del tracker (dove inviare i comandi)
            TextFormField(
          ...
          children: [
            ...
            if (setupData.timezonesList.isNotEmpty)
              DropdownButtonFormField<String>(
                ...
                items: setupData.timezonesList.map<DropdownMenuItem<String>>((TimezoneModel timezone) {
                ...
                onChanged: (String? value) {
                  if (value != null) {
                    setupData.selectedTimezone = value;
                  }
                },
                decoration: InputDecoration(
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Text(
                  localizations?.get('configurationInstructions') ?? 'Invia queste configurazioni una alla volta e attendi SMS di conferma',
...
// ✅ NUOVO STEP per configurare l'admin number correttamente
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              localizations?.get('adminDescription') ?? 'Imposta il numero di telefono che potrà inviare comandi al tracker.',
            ...
            // ✅ Campo per il numero dell'amministratore
            TextFormField(
            ...
            FilledButton.icon(
              onPressed: isSending ? null : () => onSendCommand('center'),
            ...
            Card(
              ...
              child: Padding(
                padding: const EdgeInsets.all(12),
                child: Text(
                  localizations?.get('centerCommandNote') ?? 'Questo comando imposta il numero che potrà controllare il tracker',
                  style: theme.textTheme.bodySmall?.copyWith(
                    fontStyle: FontStyle.italic,
                    color: colorScheme.onSurfaceVariant,
                  ),
        ...
        children: [
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  _SummaryItem(
                    label: localizations?.get('name') ?? 'Nome',
                    value: setupData.nameController.text,
                  ),
                  _SummaryItem(
                    label: localizations?.get('trackerPhone') ?? 'SIM Tracker',
                    value: setupData.trackerPhoneController.text,
                  ),
                  _SummaryItem(
                    label: localizations?.get('adminPhone') ?? 'Admin',
                    value: setupData.adminPhoneController.text,
                  ),
                  _SummaryItem(
                    label: localizations?.get('apn') ?? 'APN',
                    value: setupData.apnController.text,
                  ),
                  _SummaryItem(
                    label: localizations?.get('server') ?? 'Server',
                    value: '${setupData.serverHostController.text}:${setupData.serverPortController.text}',
                  ),
                  _SummaryItem(
                    label: localizations?.get('timezone') ?? 'Fuso Orario',
                    value: setupData.selectedTimezone ?? '',
                  ),
                ],
              ),
          ...
          Card(
            ...
            child: Padding(
              padding: const EdgeInsets.all(12),
              child: Text(
                localizations?.get('testInstructions') ?? 'Questo invierà i comandi WHERE e STATUS per testare la connettività',
                style: theme.textTheme.bodySmall?.copyWith(
                  fontStyle: FontStyle.italic,
                  color: colorScheme.onSurfaceVariant,
                ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          localizations?.get('setupCompleteDescription') ?? 'Il tuo tracker è ora configurato e pronto all\'uso',
                          style: theme.textTheme.bodyMedium?.copyWith(
...
class _SummaryItem extends StatelessWidget {
  final String label;  // ✅ Cambiato da 'key' a 'label'
  final String value;

  const _SummaryItem({
    super.key,           // ✅ Widget key corretto
    required this.label, // ✅ Parametro rinominato
    required this.value,
  });
...
// Quick steps for add device mode
class QuickDeviceStep extends StatelessWidget {
  final GlobalKey<FormState> formKey;
  final SetupData setupData;

  const QuickDeviceStep({
    required this.formKey,
    required this.setupData,
    super.key,
  });
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            // ✅ Anche nel quick setup, aggiungi admin number
            TextFormField(
              controller: setupData.adminPhoneController,
              decoration: InputDecoration(
                labelText: localizations?.get('adminPhoneNumber') ?? 'Numero Amministratore',
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          localizations?.get('existingConfigNote') ?? 'Userà le impostazioni APN e server esistenti',
                          style: theme.textTheme.bodyMedium?.copyWith(
                            color: Colors.green.shade700,
                          ),
                        ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(
                          localizations?.get('readyToAddDescription') ?? 'Il nuovo tracker verrà aggiunto alla lista dispositivi',
                          style: theme.textTheme.bodyMedium?.copyWith(
                            color: colorScheme.onSurface,
                          ),
                        ),
